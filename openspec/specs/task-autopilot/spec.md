# task-autopilot Specification

## Purpose

Run the CSM's open tasks autonomously through the four-band action policy: the brain loop iterates open tasks each cycle, the agent classifies each into Act / Notify-then-act / Escalate / Prep and takes the action end-to-end, outcomes feed the action ledger and digest, and the ops console surfaces the task queue and agent-driven outcomes.
